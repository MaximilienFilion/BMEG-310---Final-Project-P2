...
# RNAseq: Gene names and sample IDs
gene_names <- RNAseq_KIRC[, 1]  # First column contains gene names
rnaseq_samples <- colnames(RNAseq_KIRC)[-1]  # Remove first column (gene names)
cat("Number of genes in RNA-seq:", length(gene_names), "\n")
cat("Number of RNA-seq samples:", length(rnaseq_samples), "\n")

# Clinical: Patient IDs 
clinical_patients <- data_clinical_patient$PATIENT_ID
cat("Number of clinical patients:", length(clinical_patients), "\n")

# Mutations: Unique patient IDs (multiple rows per patient possible)
mutation_patients <- unique(data_mutations$Tumor_Sample_Barcode)
cat("Number of patients with mutation data:", length(mutation_patients), "\n\n")
```
...
```{r}
# 2. Check that sample IDs match across all datasets
cat("RNA-seq samples (first 3):\n")
print(head(rnaseq_samples, 3))
cat("\nClinical patients (first 3):\n")
print(head(clinical_patients, 3))
cat("\nMutation patients (first 3):\n")
print(head(mutation_patients, 3))
cat("\n")

# IDs don't match, so standardize them:
# For RNA-seq: convert periods to hyphens and extract patient ID
rnaseq_samples_clean <- gsub("\\.", "-", rnaseq_samples)  # Replace . with -
rnaseq_patient_ids <- substr(rnaseq_samples_clean, 1, 12)  # Get first 12 chars, remove rest

# For mutations: extract patient ID (remove sample end like -01)
mutation_patient_ids <- substr(mutation_patients, 1, 12)

# Clinical IDs should already be in correct format but rename for consistency
clinical_patient_ids <- clinical_patients
...
# -----------------DO WE THINK THE FOLLOWING IS NECESSARY? -----------------

# Boxplot of size factors
ggplot(data.frame(sample = names(sf), size_factor = sf),
       aes(x = "", y = size_factor)) +
  geom_boxplot() +
  geom_jitter(width = 0.1) +
  labs(title = "Size Factor Distribution", y = "Size Factor", x = "") +
  theme_bw()

# Identify potential outliers (rule of thumb: +/- 2 SD)
sf_mean <- mean(sf)
sf_sd <- sd(sf)
outliers <- names(sf)[abs(sf - sf_mean) > 2 * sf_sd]

cat("Potential outlier samples based on size factor:\n")
print(outliers)
cat("\n")

# 2. PCA plot (after variance stabilizing transformation)
library(DESeq2)
...
# Quality check: compare distributions before/after log transform -------- CHAT GENERATED
...
set.seed(123)
sample_cols <- sample(1:ncol(normalized_counts), 4)

# Plot 1: Density plot - Before transformation
plot(density(normalized_counts[, sample_cols[1]]), 
     main = "Density: Before Log Transformation",
     xlab = "Normalized Counts",
     ylab = "Density",
     col = "blue",
     lwd = 2,
     xlim = c(0, quantile(normalized_counts, 0.99)))
for(i in 2:4) {
  lines(density(normalized_counts[, sample_cols[i]]), 
        col = i, lwd = 2)
}
legend("topright", 
       legend = paste("Sample", 1:4),
       col = 1:4,
       lwd = 2)

# Plot 2: Density plot - After transformation
plot(density(log2_counts[, sample_cols[1]]), 
     ...
     lwd = 2)
for(i in 2:4) {
  lines(density(log2_counts[, sample_cols[i]]), 
        col = i, lwd = 2)
}
legend("topright", 
       legend = paste("Sample", 1:4),
...
# Plot 3: Histogram - Before transformation
hist(normalized_counts[, sample_cols[1]],
     breaks = 50,
     main = "Histogram: Before Log Transformation",
     xlab = "Normalized Counts",
     ylab = "Frequency",
     col = "lightblue",
     border = "white",
...
# Plot 4: Histogram - After transformation
hist(log2_counts[, sample_cols[1]],
     breaks = 50,
...
# Additional comparison: Box plots across samples
par(mfrow = c(1, 2))

# Boxplot: Before (showing subset of samples for visibility)
boxplot(normalized_counts[, sample_cols],
        main = "Before Log Transform",
        ylab = "Normalized Counts",
        xlab = "Samples",
        col = "lightblue",
        names = paste("S", 1:4, sep = ""))

# Boxplot: After
boxplot(log2_counts[, sample_cols],
        main = "After Log Transform",
        ylab = "Log2(Counts + 1)",
        xlab = "Samples",
        col = "lightgreen",
        names = paste("S", 1:4, sep = ""))
...
# Calculate mean and variance for each gene before and after transformation
mean_before <- rowMeans(normalized_counts)
var_before <- apply(normalized_counts, 1, var)

mean_after <- rowMeans(log2_counts)
var_after <- apply(log2_counts, 1, var)

# Plot mean-variance relationship
par(mfrow = c(1, 2))

plot(log10(mean_before), log10(var_before),
     main = "Mean-Variance: Before Log Transform",
     xlab = "Log10(Mean)",
     ylab = "Log10(Variance)",
     pch = 16,
     col = rgb(0, 0, 1, 0.3),
     cex = 0.5)
abline(0, 1, col = "red", lwd = 2, lty = 2)  # Reference line (variance = mean)

plot(mean_after, var_after,
     main = "Mean-Variance: After Log Transform",
     xlab = "Mean",
     ylab = "Variance",
     pch = 16,
     col = rgb(0, 1, 0, 0.3),
...
cat("Log2-transformed counts stored in 'log2_counts' object\n")
```
